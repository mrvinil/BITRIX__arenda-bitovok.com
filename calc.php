<?php
	...
	if(empty($errors)){
		...
		$message  .= "Продукт "				. $_POST['title'] ."<br />";
		$message  .= "Помещений "			. $_POST['pom'] ."<br />";
	...
	function send_mail($message){
		...
		// тема письма
		$subject = "Заявка (форма калькулятор)";
		// заголовок письма
...
$roistatData = array(
    ...
    'comment' =>'
     Продукт: '				. $_POST['title'] .'
	 Помещений: '			. $_POST['pom'] .'<br />
	 Длина: '				. $_POST['len'] .'м <br />
	 Ширина: '				. $_POST['wid'] .'м <br />
	 Внешняя отдлека: '		. $_POST['vne'] .' <br />
	 Внутренняя отделка: '  . $_POST['vnu'] .' <br />
	 Утепление: '           . $_POST['ytep'] .' <br />
	 Электрика: '			. $_POST['elect'] .' <br />
	 Фундамент: '			. $_POST['fund'] .' <br />',
    'name'    => $_POST['n'],
    'phone'   => $_POST['p'],
