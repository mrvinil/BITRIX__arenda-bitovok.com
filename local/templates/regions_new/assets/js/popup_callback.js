$(document).ready(() => {
    ...
    $('.btn[data-target=callback]').on('click', function (event) {
        ...
        const $cardDesc = $(this).siblings('.tabs-product-info');
        $productName.val($cardDesc.find('.tabs-product-info__title').text());
        $productPrice.val($cardDesc.find('.tabs-product-info__price').text());
        $productSize.val($cardDesc.find('.tabs-product-info__size').text());
        callbackOpen();
    });
